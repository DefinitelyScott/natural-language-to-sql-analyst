"""The offline backend's question catalog: the rules themselves.

Separated from :mod:`nl2sql.llm` because this module is *data* and that one is
*logic*. ``OfflineBackend`` is about seventy lines of matching and diagnostics;
the catalog it scans is two thousand lines of regex/SQL pairs plus the reasoning
for each. Holding both in one file made the class read as though it were far
more complicated than it is, and buried the four methods that actually decide
anything under the table they consult.

The split is along the axis that changes: adding a question pattern touches this
file only, and changing how questions are routed touches ``llm.py`` only. Nothing
here imports from ``llm``, so the dependency runs one way.

Ordering is load-bearing and is preserved exactly. ``OfflineBackend`` resolves a
question first-rule-wins, so a rule's position in this list is its matching
priority, and narrow rules are registered ahead of the broad ones that would
otherwise shadow them. Each entry's comment records why it sits where it does.
``tests/test_rule_catalog.py`` asserts that every rule is still reachable, which
is what makes a reordering here fail loudly rather than silently.
"""
...
#: Phrases that ask for a *breakdown* — one row per group — rather than a single
#: number: "by region", "per category", "in each region", "for every month".
#: Used only as the body of :data:`_UNGROUPED_ONLY`.
#:
#: The dimensions are enumerated rather than matched as a bare ``by \w+``,
#: because "by" is not reliably a grouping word in English: "orders placed by
#: customers" and "revenue by the end of March" both carry it without asking
#: for a GROUP BY. Listing the columns this schema can actually group on keeps
#: the guard from declining questions it has no business declining.
_GROUPED = (
    r"\b(?:by|per|across)\s+(?:each\s+|every\s+)?"
    r"(?:regions?|categor(?:y|ies)|months?|quarters?|years?|weeks?|days?"
    r"|products?|customers?|segments?|tiers?)\b"
    r"|\b(?:in|for|within|across)\s+(?:each|every)\b"
    r"|\beach\s+(?:regions?|categor(?:y|ies)|months?|quarters?|years?|weeks?"
    r"|days?|products?|customers?)\b"
)

#: Prefix for a rule whose SQL returns one row for the whole table, e.g.
#: ``SELECT COUNT(*) FROM customers``.
#:
#: The sibling of :data:`_UNSCOPED_ONLY`, guarding the same catch-all rules
#: against the same failure in the other dimension. Those rules are phrased
#: broadly ("how many customers"), so "how many customers are in each region?"
#: used to fall through to the customer-count rule and be answered with the
#: single number 120 — a plausible, correctly labelled figure that silently
#: drops the grouping the question was entirely about.
#:
#: Grouping the catalog *does* implement is unaffected, for the same reason
#: periods are: "revenue by category" and "average order value by region" have
#: their own rules registered earlier, and first-match resolution reaches them
#: before these. What this guard changes is only the fall-through: a breakdown
#: the catalog cannot compute now raises ``NoRuleMatchError`` and gets the CLI's
#: nearest-question suggestions, instead of a total wearing a breakdown's label.
_UNGROUPED_ONLY = rf"\A(?!.*(?:{_GROUPED}))"


def build_catalog() -> list[tuple[re.Pattern[str], str]]:
    """Return the ordered rule catalog: one ``(matcher, sql)`` pair per question.

    Built fresh on each call rather than held as a module-level constant, so no
    caller can mutate a list every :class:`~nl2sql.llm.OfflineBackend` shares.
    Compiling the patterns costs microseconds and happens once per backend, not
    once per question.
    """
    return [
        ...
        # Customer headcount by region: the grouped counterpart to the
        # whole-table customer count below, and the reason that rule now
        # carries _UNGROUPED_ONLY. "How many customers are in each region?"
        # used to reach the count rule and come back as one number, so the
        # guard alone would have turned a wrong answer into no answer. This
        # rule is what makes the most natural phrasing of that question an
        # answered one; the guard covers the breakdowns the catalog still
        # cannot compute.
        #
        # Registered immediately ahead of the count rule rather than beside
        # the other region rules further down, because adjacency is what
        # makes the pair legible: the guard on one and the existence of the
        # other are a single decision, and splitting them across 300 lines
        # would leave each looking arbitrary.
        #
        # The share column is carried because a headcount split four ways is
        # read as a distribution — "is the West underweight?" — and doing
        # that arithmetic by eye against a total the table does not show is
        # exactly the step this tool exists to remove. The denominator is a
        # scalar subquery over the whole table, not SUM(COUNT(*)) OVER (),
        # so the percentages stay against total customers even if a WHERE
        # clause is later added to the outer query.
        #
        # ORDER BY breaks ties on region: four regions of 120 customers make
        # an exact tie ordinary, and an unordered tie would make the row
        # order — and so the eval's row-for-row comparison — depend on
        # SQLite's grouping order rather than on the data.
        (
            re.compile(
                r"\b(?:how many|number of|count of)\s+(?:customers?|users?)\b"
                r"[^?]*\b(?:by|per|across|(?:in|for|within)\s+(?:each|every)"
                r"|each)\s+regions?\b"
                r"|\bcustomers?\s+(?:by|per)\s+regions?\b"
                r"|\bcustomers?\s+counts?\b[^?]*\b(?:by|per)\s+regions?\b"
                r"|\bwhich\s+regions?\b[^?]*"
                r"\b(?:most|fewest|largest|smallest|highest|lowest)\s+"
                r"(?:number\s+of\s+)?customers?\b",
                re.I,
            ),
            """
            SELECT c.region AS region,
                   ...
                   ROUND(
                       100.0 * COUNT(*) / (SELECT COUNT(*) FROM customers), 1
                   ) AS pct_of_customers
            FROM customers c
            GROUP BY c.region
        ...
        (
            ...
            # not a question it can answer. Guarded by _UNGROUPED_ONLY for
            # the same reason in the other dimension: it returns one number,
            # so "how many customers are in each category?" is not one
            # either — the region breakdown above is the only grouping of
            # this count the catalog implements.
            re.compile(
                _UNSCOPED_ONLY
                + _UNGROUPED_ONLY
                + r".*how many (?:customers|users)",
                re.I,
            ),
        ...
        #
        # Each also carries the _UNSCOPED_ONLY and _UNGROUPED_ONLY guards:
        # being both last and broad is what makes them the catch-all that a
        # period-scoped or grouped question lands on, and every one of them
        # collapses a whole table to a single row with no date filter and no
        # GROUP BY. See those two constants for why declining beats
        # answering there.
        (
            re.compile(
                _UNSCOPED_ONLY
